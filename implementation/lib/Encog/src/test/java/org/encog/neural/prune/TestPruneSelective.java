...
public class TestPruneSelective extends TestCase {
    ...
    private void checkWithModel(FlatNetwork model, FlatNetwork pruned) {
        Assert.assertEquals(model.getWeights().length,
                            pruned.getWeights().length);
        Assert.assertArrayEquals(model.getContextTargetOffset(), pruned
                .getContextTargetOffset());
        Assert.assertArrayEquals(model.getContextTargetSize(), pruned
                .getContextTargetSize());
        Assert
                .assertArrayEquals(model.getLayerCounts(), pruned
                .getLayerCounts());
        Assert.assertArrayEquals(model.getLayerFeedCounts(), pruned
                .getLayerFeedCounts());
        Assert.assertArrayEquals(model.getLayerIndex(), pruned.getLayerIndex());
        Assert.assertEquals(model.getLayerOutput().length, pruned
                .getLayerOutput().length);
        Assert
                .assertArrayEquals(model.getWeightIndex(), pruned
                .getWeightIndex());
    }
    ...
    public void testPruneNeuronInput() {
        ...
        checkWithModel(model.getStructure().getFlat(), network.getStructure()
                .getFlat());
    ...
    public void testPruneNeuronHidden() {
        ...
        Assert.assertEquals("1,3,4,5,7,8,9,11,12,13,15,16,17,18,19,23,24,25",
                            network.dumpWeights());
    ...
    public void testRandomizeNeuronInput() {
        ...
        Assert.assertEquals("0,0,0,0,0,100,0,0,100,0,0,100,0", network
                .dumpWeights());
    }
    ...
    public void testRandomizeNeuronHidden() {
        ...
        Assert.assertEquals("0,100,0,0,0,0,0,100,100,100,0,0,0", network
                .dumpWeights());
    }
    ...
    public void testRandomizeNeuronOutput() {
        ...
        Assert.assertEquals("100,100,100,100,0,0,0,0,0,0,0,0,0", network
                .dumpWeights());
    }
}
