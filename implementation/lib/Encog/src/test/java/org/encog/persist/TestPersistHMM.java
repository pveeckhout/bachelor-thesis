...
public class TestPersistHMM extends TestCase {
    ...
    static HiddenMarkovModel buildContHMM() {
        ...
        hmm.setStateDistribution(0,
                                 new ContinousDistribution(mean1, covariance1));
        hmm.setStateDistribution(1,
                                 new ContinousDistribution(mean2, covariance2));
    ...
    static HiddenMarkovModel buildDiscHMM() {
        ...
        hmm.setStateDistribution(0, new DiscreteDistribution(new double[][]{{
                0.95, 0.05}}));
        hmm.setStateDistribution(1, new DiscreteDistribution(new double[][]{{
                0.20, 0.80}}));
    ...
    public void testDiscPersistEG() {
        ...
        HiddenMarkovModel resultHMM =
                (HiddenMarkovModel) EncogDirectoryPersistence.loadObject(
                EG_FILENAME);

        validate(resultHMM, sourceHMM);
    }

    public void testDiscPersistSerial() throws IOException,
            ClassNotFoundException {
        HiddenMarkovModel sourceHMM = buildDiscHMM();

        SerializeObject.save(SERIAL_FILENAME, sourceHMM);
        HiddenMarkovModel resultHMM = (HiddenMarkovModel) SerializeObject.load(
                SERIAL_FILENAME);
    ...
    public void testContPersistSerial() throws IOException,
            ClassNotFoundException {
        HiddenMarkovModel sourceHMM = buildContHMM();
