...
public class AnalystProcess extends BasicFile {
    ...
    private final EncogProgramVariables programVariables =
            new EncogProgramVariables();
    private final List<EncogProgram> expressionFields =
            new ArrayList<EncogProgram>();
     ...
     */
    public AnalystProcess(EncogAnalyst theAnalyst, int theBackwardWindowSize,
    ...
    public void analyze(final File inputFile, final boolean headers,
        ...
        for (ProcessField field : this.analyst.getScript().getProcess()
                .getFields()) {
            EncogProgram prg = new EncogProgram(this.programContext,
                                                this.programVariables);
            prg.setExtraData(ProcessExtension.EXTENSION_DATA_NAME,
                             this.extension);
