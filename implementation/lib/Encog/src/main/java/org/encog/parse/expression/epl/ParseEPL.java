...
public class ParseEPL {
    ...
    private StackObject<ProgramNode> nodeStack = new StackObject<ProgramNode>(
            100);
    ...
    public ProgramNode parse(final String expression) {
        ...
        while (!this.parser.eol()) {
            ...
            // factor the node
            ProgramNode node = this.holder.getFunctions()
                    .factorProgramNode(name, this.holder, args);
            ...
            for (int i = 0; i < temp.getDataSize(); i++) {
                ...
                if (idx != -1) {
                ...
                else if (str.length() == 1 && "tf".indexOf(Character
                        .toLowerCase(str.charAt(0))) != -1) {
                    node.getData()[i] = new ExpressionValue(str
                            .equalsIgnoreCase("t"));
                } // is it a string?
                else if (str.charAt(0) == '\"') {
                    node.getData()[i] = new ExpressionValue(str.substring(1, str
                            .length() - 1));
                } // is it an integer
                else if (str.indexOf('.') == -1 && str.toLowerCase()
                        .indexOf('e') == -1) {
                    long l;
                    try {
                    ...
                    node.getData()[i] = new ExpressionValue(CSVFormat.EG_FORMAT
                            .parse(str));
                }
            }
        }
