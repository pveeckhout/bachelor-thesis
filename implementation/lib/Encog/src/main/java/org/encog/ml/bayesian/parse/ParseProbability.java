...
public class ParseProbability {
     ...
     */
    private void addEvents(SimpleParser parser, List<ParsedEvent> results,
                           String delim) {
        boolean done = false;
                ...
                if (ch == '[') {
                    ...
                    while (ch != ']' && !parser.eol()) {
                        ...
                        if (parser.peek() == ':') {
                            ...
                            double min = Double.parseDouble(parser
                                    .readToWhiteSpace());
                            parser.eatWhiteSpace();
                            ...
                            parser.advance(2);
                            double max = CSVFormat.EG_FORMAT.parse(parser
                                    .readToChars(",]"));
...
//					BayesianEvent evt = this.network.getEvent(parsedEvent.getLabel());
    ...
    public static List<ParsedProbability> parseProbabilityList(
            BayesianNetwork network, String line) {
        ...
        for (int i = 0; i < line.length(); i++) {
            ...
            if (ch == ')') {
                ...
                ParsedProbability parsedProbability = parse.parse(prob
                        .toString());
