...
public class NeuralPSO extends BasicTraining {
    ...
    protected void updateVelocity(int particleIndex, double[] particlePosition) {
        ...
        if (i != m_bestVectorIndex) {
            m_va.copy(vtmp, m_pseudoAsynchronousUpdate
                    ? m_bestVectors[m_bestVectorIndex] : m_bestVector);
            m_va.sub(vtmp, particlePosition);
    ...
    protected void updatePersonalBestPosition(int particleIndex,
        ...
        double score = m_calculateScore
                .calculateScore(m_networks[particleIndex]);

        // update the best vectors (g and i)
        if ((m_bestErrors[particleIndex] == 0) || isScoreBetter(score,
                                                                m_bestErrors[particleIndex])) {
    ...
    protected void updateGlobalBestPosition() {
        ...
        for (int i = 0; i < m_populationSize; i++) {
            if ((m_bestVectorIndex == -1) || isScoreBetter(m_bestErrors[i],
                                                           m_bestErrors[m_bestVectorIndex])) {
                m_bestVectorIndex = i;
                bestUpdated = true;
            }
        }
    ...
    public String getDescription() {
        return String.format(
                "pop = %d, w = %.2f, c1 = %.2f, c2 = %.2f, Xmax = %.2f, Vmax = %.2f",
                m_populationSize, m_inertiaWeight, m_c1, m_c2, m_maxPosition,
                m_maxVelocity);
    }
