 ...
 */
public class BayesianNetwork extends BasicML implements MLClassification,
     ...
     */
    private final Map<String, BayesianEvent> eventMap =
            new HashMap<String, BayesianEvent>();
    /**
    ...
    public void setContents(String line) {
        List<ParsedProbability> list = ParseProbability.parseProbabilityList(
                this, line);
    ...
    public int classify(MLData input) {
        ...
        if (this.classificationTarget < 0 || this.classificationTarget >=
                this.events.size()) {
            throw new BayesianError(
    ...
    public String getClassificationStructure() {
        ...
        for (int i = 0; i < this.getEvents().size(); i++) {
            BayesianEvent event = this.events.get(i);
            if (this.query.getEventState(event).getEventType() ==
                    EventType.Evidence) {
                if (!first) {
                    result.append(",");
                }
