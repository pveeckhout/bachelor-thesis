...
public class FreeformNetwork extends BasicML implements MLContext, Cloneable,
     ...
     */
    private final FreeformConnectionFactory connectionFactory =
            new BasicFreeformConnectionFactory();
    /**
     * The layer factory.
     */
    private final FreeformLayerFactory layerFactory =
            new BasicFreeformLayerFactory();
    /**
     * The neuron factory.
     */
    private final FreeformNeuronFactory neuronFactory =
            new BasicFreeformNeuronFactory();
    /**
     * The input summation factory.
     */
    private final InputSummationFactory summationFactory =
            new BasicActivationSummationFactory();
    ...
    public FreeformNetwork(final BasicNetwork network) {
        ...
        for (int currentLayerIndex = 0; currentLayerIndex < network
            ...
            for (int i = 0; i < network.getLayerNeuronCount(currentLayerIndex);
                    i++) {
            ...
            if (previousLayer != null) {
                connectLayersFromBasic(network, currentLayerIndex - 1,
                                       previousLayer, currentLayerIndex,
                                       currentLayer,
    ...
    private void connectLayersFromBasic(final BasicNetwork network,
                                        final int fromLayerIdx,
                                        final FreeformLayer source,
                                        final int sourceIdx,
                                        final FreeformLayer target,
                                        final int targetIdx,
                                        final boolean isRecurrent) {

        for (int targetNeuronIdx = 0; targetNeuronIdx < target.size();
                targetNeuronIdx++) {
            for (int sourceNeuronIdx = 0; sourceNeuronIdx < source.size();
                    sourceNeuronIdx++) {
    ...
    private void performConnectionTask(final Set<FreeformNeuron> visited,
                                       final FreeformNeuron parentNeuron,
     ...
     * @param connectionSize The number of elements to allocate on each
     *                       connection.
     */
