...
public class TestNormArray extends TestCase {
    ...
    private DataNormalization create1D(double[] arrayOutput) {
        ...
        NormalizationStorageArray1D target = new NormalizationStorageArray1D(
                arrayOutput);
    ...
    private DataNormalization create2D(double[][] arrayOutput) {
        ...
        NormalizationStorageArray2D target = new NormalizationStorageArray2D(
                arrayOutput);
    ...
    public void testArray1DSerial() throws Exception {
        ...
        arrayOutput = ((NormalizationStorageArray1D) norm.getStorage())
                .getArray();
        norm.process();
        check1D(arrayOutput);
    }
    ...
    public void testArray2DSerial() throws Exception {
        ...
        DataNormalization norm = create2D(arrayOutput);
        norm = (DataNormalization) SerializeRoundTrip.roundTrip(norm);
        arrayOutput = ((NormalizationStorageArray2D) norm.getStorage())
                .getArray();
        norm.process();
        check2D(arrayOutput);
    }
