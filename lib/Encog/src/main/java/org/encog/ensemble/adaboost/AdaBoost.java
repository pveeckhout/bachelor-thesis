...
public class AdaBoost extends Ensemble {
    ...
    public AdaBoost(int iterationsT, int dataSetSize,
    ...
    @Override
    public void train(double targetAccuracy, double selectionError,
        ...
        for (int i = 0; i < T; i++) {
            ...
            GenericEnsembleML newML = new GenericEnsembleML(mlFactory.createML(
                    dataSetFactory.getInputData().getInputSize(), dataSetFactory
                    .getInputData().getIdealSize()), mlFactory.getLabel());
            do {
    ...
    private ArrayList<Double> updateD(GenericEnsembleML ml, MLDataSet dataSet,
                                      ArrayList<Double> D_t) {
        ...
        for (int i = 0; i < dataSet.size(); i++) {
            double D_tplus1_i = D_t.get(i) * Math.exp(-alpha_t * va.dotProduct(
                    dataSet.get(i).getIdeal().getData(), ml.compute(dataSet.get(
                    i).getInput()).getData()));
    ...
    private double getWeightedError(GenericEnsembleML newML, MLDataSet dataSet) {
        ...
        for (int i = 0; i < dataSet.size(); i++) {
            ...
            if (newML.classify(currentData.getInput()) != newML.winner(
                    currentData.getIdeal())) {
