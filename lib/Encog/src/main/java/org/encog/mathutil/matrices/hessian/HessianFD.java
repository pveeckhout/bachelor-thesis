...
public class HessianFD extends BasicHessian {
    ...
    public void init(BasicNetwork theNetwork, MLDataSet theTraining) {
        ...
        int weightCount =
                theNetwork.getStructure().getFlat().getWeights().length;
    ...
    private void internalCompute(int outputNeuron) {
        ...
        for (final MLDataPair pair : this.training) {
            ...
            e = pair.getIdeal().getData(outputNeuron) - networkOutput.getData(
                    outputNeuron);
            ...
            int outputFeedCount = network.getLayerTotalNeuronCount(network
                    .getLayerCount() - 2);
     ...
     */
    private double computeDerivative(final MLData inputData, int outputNeuron,
                                     int weight, final double[] stepSize,
