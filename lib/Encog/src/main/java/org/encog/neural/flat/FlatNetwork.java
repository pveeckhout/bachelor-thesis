...
public class FlatNetwork implements Serializable, Cloneable {
    ...
    public double calculateError(final MLDataSet data) {
        ...
        for (int i = 0; i < data.getRecordCount(); i++) {
            ...
            errorCalculation.updateError(actual, pair.getIdealArray(), pair
                    .getSignificance());
        }
    ...
    public void clearContext() {
        ...
        for (int i = 0; i < this.layerIndex.length; i++) {
            ...
            final boolean hasBias = (this.layerContextCount[i] +
                    this.layerFeedCounts[i]) != this.layerCounts[i];

            // fill in regular neurons
            Arrays
                    .fill(this.layerOutput, index, index +
    ...
    public void cloneFlatNetwork(final FlatNetwork result) {
        ...
        result.activationFunctions =
                new ActivationFunction[this.activationFunctions.length];
        ...
        for (int i = layers.length - 1; i >= 0; i--) {
            ...
            if (index == 0) {
                ...
                this.weightIndex[index] = this.weightIndex[index - 1] +
                        (this.layerCounts[index] * this.layerFeedCounts[index -
                        1]);
