...
public class CmdCreate extends Cmd {
    ...
    private void handlePrgPopulation(PrgPopulation pop) {
        ...
        for (AnalystField field : getScript().getNormalize()
                .getNormalizedFields()) {
            ...
            switch (field.getAction()) {
                ...
                case PassThrough:
                    if (df.isInteger()) {
                        mapping =
                                new VariableMapping(varName, ValueType.intType);
                    } else if (df.isReal()) {
                        mapping = new VariableMapping(varName,
                ...
                case SingleField:
                    ...
                    break;
                default:
                    throw new AnalystError("Unknown normalization action: " +
        ...
        RampedHalfAndHalf generate = new RampedHalfAndHalf(pop.getContext(), 1,
                                                           6);
