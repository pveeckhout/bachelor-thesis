...
public class GenerateEncogJava extends AbstractGenerator {
    ...
    @Override
    public void generate(final EncogGenProgram program,
                         final boolean shouldEmbed) {
    ...
    private void generateLoadTraining(final EncogProgramNode node) {
        ...
        if (this.embed) {
            ...
            line.append(
                    "MLDataSet result = new BasicMLDataSet(INPUT_DATA,IDEAL_DATA);");
        } else {
            addInclude("org.encog.util.simple.EncogUtility");
            line.append(
                    "MLDataSet result = EncogUtility.loadEGB2Memory(new File(\"");
    ...
    private void linkNetwork(final EncogProgramNode node) {
        ...
        line.setLength(0);
        line.append(
                "MLMethod result = (MLMethod)EncogDirectoryPersistence.loadObject(new File(\"");
        line.append(methodFile.getAbsolutePath());
        line.append("\"));");
        addLine(line.toString());
