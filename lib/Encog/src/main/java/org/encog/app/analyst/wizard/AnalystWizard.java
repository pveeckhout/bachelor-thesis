...
public class AnalystWizard {
    ...
    private void generateFeedForward(final int inputColumns,
        ...
        final int hidden = (int) ((inputColumns * Math
                .max(this.lagWindowSize, 1)) * 1.5);
    ...
    private void generateEPL(final int inputColumns,
        ...
        if (inputColumns > 26) {
            throw new EncogError(
                    "More than 26 input variables is not supported for EPL.");
    ...
    private void generateNormalizedFields() {
        ...
        for (int i = 0; i < this.script.getFields().length; i++) {
            ...
            if ((this.methodType == WizardMethodType.EPL) || (this.methodType ==
                    WizardMethodType.BayesianNetwork)) {
                AnalystField af;
                if (f.isClass()) {
    ...
    public void wizardRealTime(List<SourceElement> sourceData, File csvFile,
                               int backwardWindow, int forwardWindow,
    ...
    public void wizard(final File analyzeFile, final boolean b,
        ...
        this.timeSeries =
                ((this.lagWindowSize > 0) || (this.leadWindowSize > 0));
        this.format = format;
